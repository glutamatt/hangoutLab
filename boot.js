...
document.write('<link href="' + webRoot + 'styles.css" rel="stylesheet" media="screen">');
document.write('<canvas id="world" width="900" height="600"></canvas>');
...
$(function(){
	init(gapi.hangout.getParticipants());
});

var participantsPosition = {};
...
function init(participants) {
	...
	if (canvas && canvas.getContext) {
		...
		//initialiser les joueurs existant
		for (var index in participants)
			addParticipant(participants[index].id);
		...
		setInterval(function(){
			gapi.hangout.data.setValue(gapi.hangout.getLocalParticipantId(), JSON.stringify([mouseX, mouseY]));
		}, 500);
		
		//enregistrer la position des joueurs recu via state
		gapi.hangout.data.onStateChanged.add( function(event) {
			for (var key in event.state)
				participantsPosition[key] = JSON.parse(event.state[key]);
		} );
		
		//gérer l'ajout de nouveaux participants
		gapi.hangout.onParticipantsAdded.add(function(event){
			for (var index in event.addedParticipants)
				addParticipant(event.addedParticipants[index].id);
		});
	}
}
...
{
	participantsPosition[participantId] = [mouseX, mouseY] ;
	createParticles(participantId);
}

function createParticles(participantId) {
...
function loop() {
	...
	for (i = 0, len = particles.length; i < len; i++) {
		...
		particle.shift.x += ( participantsPosition[particle.participantId][0] - particle.shift.x) * (particle.speed);
		particle.shift.y += ( participantsPosition[particle.participantId][1] - particle.shift.y) * (particle.speed);
